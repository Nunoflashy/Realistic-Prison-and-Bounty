...
;/
    Reads a preset file, returning its raw bucket-keyed contents (FastMap<string, object>,
    keyed by bucket name - e.g. "General", a Hold name).

    Returns PRESET_INVALID_FILE()/PRESET_NOT_FOUND() (both negative, never a real object id) on
    failure - check the result against those before treating it as a real FastMap.

    string  @asPresetFile: The name of the preset file to load (relative to Preset_GetDirectory()).
/;
...
;/
    Writes a bucket-keyed data blob to a preset file (to build one, put each bucket's own data
    object into a FastMap<string> keyed by bucket name - see RPB_MCM.SavePreset).

    int     @apBucketsData: FastMap<string, object> keyed by bucket name.
    string  @asPresetFile: The preset file name to write (relative to Preset_GetDirectory()).
/;
function Preset_Save(int apBucketsData, string asPresetFile) global
    if (!apBucketsData)
        Debug("Data::Preset_Save", "Failed to save preset: Invalid data content.")
        return
    endif

    Object_WriteData(apBucketsData, Preset_GetDirectory() + "/" + asPresetFile)
    Debug("Data::Preset_Save", "Saved preset: " + asPresetFile)
endFunction

;/
    Validates and applies exactly one bucket's preset data into the destination content object.
    Each key is checked against that bucket's registrant (registered by RPB_MCM.RegisterPages())
    before being applied - a key that's no longer a real, live option for this bucket is skipped
    and logged, not blindly written.

    string  @asBucket: The bucket name (e.g. "General", a Hold name) this data belongs to.
    int     @apSrcBucketData: FastMap<string, object> - this bucket's own data, as read from the preset.
    int     @apDstDataContent: FastMap<string, object> - the destination bucket map to write into.
...
function Preset_ApplyBucket(string asBucket, int apSrcBucketData, int apDstDataContent, bool abVerbose = true, bool abDryRun = false) global
    if (!apSrcBucketData)
        Debug("Data::Preset_ApplyBucket", "Failed to apply preset bucket " + asBucket + ": Invalid source data content.")
        return
    endif

    if (!apDstDataContent)
        Debug("Data::Preset_ApplyBucket", "Failed to apply preset bucket " + asBucket + ": Invalid destination data content.")
        return
    endif

    ; Mirrors RPB_MCM.psc's TYPE_INT/TYPE_FLOAT/TYPE_STRING properties (2/3/6) - not accessible here
    ; (they're properties on an RPB_MCM instance, and this function has no such reference).
    int TYPE_INT    = 2
    int TYPE_FLOAT  = 3
    int TYPE_STRING = 6

    string registrant  = RPB_Registry.AsPresetRegistrantKey(asBucket)
    string[] srcKeys    = FastMap_KeysAsPapyrusArray(apSrcBucketData)
    ...
    while (i < srcKeys.Length)
        ...
        if (!RPB_Registry.HasContent(presetContentKey, registrant))
            ; Debug("Data::Preset_ApplyBucket", "Skipped invalid content for bucket " + asBucket + ": " + presetContentKey, abCondition = abVerbose)
            skippedCount += 1
        else
            ; Every real option value is a primitive (bools stored as int 0/1) - never an object -
            ; so this must copy with the matching typed getter/setter, not FastMap_GetObject/SetObject
            ; (the wrong accessor for a primitive value; it silently returns/writes 0).
            int valueType = FastMap_ValueType(apSrcBucketData, presetContentKey)

            if (!abDryRun)
                if (valueType == TYPE_INT)
                    FastMap_SetInt(apDstDataContent, presetContentKey, FastMap_GetInt(apSrcBucketData, presetContentKey))
                elseif (valueType == TYPE_FLOAT)
                    FastMap_SetFloat(apDstDataContent, presetContentKey, FastMap_GetFloat(apSrcBucketData, presetContentKey))
                elseif (valueType == TYPE_STRING)
                    FastMap_SetString(apDstDataContent, presetContentKey, FastMap_GetString(apSrcBucketData, presetContentKey))
                endif

                appliedCount += 1
            endif

            ; Debug("Data::Preset_ApplyBucket", "Applied content for bucket " + asBucket + ": " + presetContentKey, abCondition = !abDryRun && abVerbose)
            ; Debug("Data::Preset_ApplyBucket", "(Dry Run) Would apply content for bucket " + asBucket + ": " + presetContentKey, abCondition = abDryRun && abVerbose)
        endif

        i += 1
    endWhile

    Debug("Data::Preset_ApplyBucket", "Bucket " + asBucket + " completed: " + appliedCount + " applied, " + skippedCount + " skipped.", abCondition = !abDryRun && abVerbose)
endFunction
...
int function GetPrisonObject(string asPath = "/") global
    string path = GetModDataDirectory() + asPath
    int root = JValue.readFromFile(GetModDataDirectory() + asPath)
    return root
endFunction

string[] function GetRootObjectInPath(string asPath = "/") global
    string path = GetModDataDirectory() + asPath
    return JContainers.contentsOfDirectoryAtPath(path)
    ; int root = JValue.readFromDirectory(GetModDataDirectory() + asPath)
    ; return root
endFunction
...
Form[] function FindPropertyOfTypeFormArray(int apRootObject, string asPropertyPath, string apFindConditions = "[]", Form[] akDefaultValue = none, bool abCheckExists = true, string asPathDelimiter = "//") global
    ...
    int parentObject = apRootObject
    ; DebugWithArgs("Data::FindPropertyOfTypeFormArray", asPropertyPath, \
    ;     "\n\t pathKey: "            + pathKey + \
    ;     "\n\t subCategories: "      + subCategories + \
    ;     "\n\t isMap: "              + JValue.isMap(parentObject) + \
    ;     "\n\t isFormMap: "          + JValue.isFormMap(parentObject) + \
    ...
    if (JValue.isFormMap(parentObject))
        returnedObject = JFormMap.allKeys(parentObject)
        int formArrayLength = JValue.count(returnedObject)

        int newFormArray = JArray.object()

        int i = 0
        while (i < formArrayLength)
            Form childKey = JFormMap.getNthKey(parentObject, i)
            int childObject = JFormMap.getObj(parentObject, childKey)
            bool conditionsMet = CheckFindConditions(childObject, NormalizeJSON(apFindConditions))
            if (conditionsMet)
                Form currentForm = JFormMap.getNthKey(parentObject, i)
                JArray.addForm(newFormArray, currentForm)
            endif
            i += 1
        endWhile

        return JArray.asFormArray(newFormArray)
    endif
...
function LogComparison(int conditionValueType, string conditionKey, string conditionValue, int apObject) global
    int actualValueType = JValue.solvedValueType(apObject, "." + conditionKey)
    ...
    if (actualValueType == TYPE_INT)
        actualValue = JMap.getInt(apObject, conditionKey) as string

    elseif (actualValueType == TYPE_FLOAT)
        actualValue = JMap.getFlt(apObject, conditionKey) as string

    elseif (actualValueType == TYPE_STRING)
        actualValue = JMap.getStr(apObject, conditionKey)

    endif

    Debug("[Condition Value Type: " + conditionValueType + "] [Actual Value Type: "+ actualValueType +"] Data::CheckFindConditions", "conditionKey: " + conditionKey + ", conditionValue: " + conditionValue + ", actualValue: " + actualValue);
endFunction
...
; TODO: Implement various object handlers
...
;                 if (JValue.isMap(nestedConditionObject) && !CheckMapConditions(nestedActualObject, nestedConditionObject))
;                     return false

;                 elseif (JValue.isIntegerMap(nestedConditionObject) && !CheckIntegerMapConditions(nestedActualObject, nestedConditionObject))
;                     return false

;                 elseif (JValue.isFormMap(nestedConditionObject) && !CheckFormMapConditions(nestedActualObject, nestedConditionObject))
;                     return false

;                 elseif (JValue.isArray(nestedConditionObject) && !CheckArrayConditions(nestedActualObject, nestedConditionObject))
;                     return false
;                 endif
...
bool function CheckMapConditions(int apActualMap, int apConditionMap, bool abCheckMissingKeys = true) global
    int mapLen = JValue.count(apConditionMap)
    int i = 0
    while (i < mapLen)
        string conditionKey     = JMap.getNthKey(apConditionMap, i)
        string conditionValue   = JMap.getStr(apConditionMap, conditionKey)
        string actualValue      = JMap.getStr(apActualMap, conditionKey)
...
bool function CheckIntegerMapConditions(int apActualMap, int apConditionMap, bool abCheckMissingKeys = true) global
    ...
    int i = 0
    while (i < mapLen)
        int conditionKey     = JIntMap.getNthKey(apConditionMap, i)
        int conditionValue   = JIntMap.getInt(apConditionMap, conditionKey)
        int actualValue      = JIntMap.getInt(apActualMap, conditionKey)
...
bool function CheckFormMapConditions(int apActualMap, int apConditionMap, bool abCheckMissingKeys = true) global
    ...
    int i = 0
    while (i < mapLen)
        Form conditionKey     = JFormMap.getNthKey(apConditionMap, i)
        string conditionValue = JFormMap.getStr(apConditionMap, conditionKey)
        string actualValue    = JFormMap.getStr(apActualMap, conditionKey)
...
bool function CheckArrayConditions(int apActualArray, int apConditionArray, bool abCheckMissingKeys = true) global
    int arrLen = JValue.count(apConditionArray)
    int i = 0
    while (i < arrLen)
        int conditionValue   = JArray.getObj(apConditionArray, i)
        int actualValue      = JArray.getObj(apActualArray, i)

        if (!CheckConditionsRecursively(actualValue, conditionValue, abCheckMissingKeys))
            return false
        endif
