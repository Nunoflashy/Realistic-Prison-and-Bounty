...
bool function ReloadData()
    ...
    if (isValidData)
        ...
        Debug("["+ self +"] SerializableObjectReference::ReloadData", "Data has been reloaded.", !self.IsDataHotLoadable)
        DebugWarn("["+ self +"] SerializableObjectReference::ReloadData", "Data has been reloaded, but the reference is already hot loadable!", self.IsDataHotLoadable)
    else
        DebugError("["+ self +"] SerializableObjectReference::ReloadData", "There are errors in the data object, cannot reload data!", !isValidData)
