...
function OnOptionHighlight(RealisticPrisonAndBounty_MCM mcm, int oid, int index) global
    ...
    if (oid == mcm.oid_release_giveItemsBackOnRelease[index])
    ...
    ;     mcm.SetInfoText("Determines whether the items should be given back when defeated and released from " + holds[index] + " prison.")
    endif
endFunction
